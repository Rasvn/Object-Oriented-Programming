...
public:
	...
	// Dynamic Vector Class Copy Constructor
	...
	// Swaps 2 elements from the vector
	void swap(unsigned i, unsigned j);
	// Returns the iterator of the Dynamic Vector
	DynamicVectorIterator<Type> iterator();
	// Returns the iterator to the first element of the Dynamic Vector
	DynamicVectorIterator<Type> begin() noexcept {
		return DynamicVectorIterator<Type>(*this);
	}
	// Returns the first invalid element of the Dynamic Vector
	DynamicVectorIterator<Type> end() noexcept {
		return DynamicVectorIterator<Type>(*this, length);
	}
};

template <typename Type>
void DynamicVector<Type>::redim() {
	auto newElements = make_unique<Type[]>(capacity * 2);
	for (unsigned i = 0; i < length; ++i) {
...
template <typename Type>
void DynamicVector<Type>::swap(unsigned i, unsigned j) {
	if (i >= length || j >= length) {
		throw exception("Invalid position!");
	}
	Type aux = move(elements[i]);
	elements[i] = move(elements[j]);
	elements[j] = move(aux);
}
...
private:
	...
	// Dynamic Vector Iterator Position Constructor
	DynamicVectorIterator(const DynamicVector<Type>&, unsigned);
...
template <typename Type>
DynamicVectorIterator<Type>::DynamicVectorIterator(const DynamicVector<Type>& _v, unsigned position) : v(_v) {
	curent = position;
}
...
template <typename Type>
const Type& DynamicVectorIterator<Type>::element() const {
